...
class CommonTests(AttendanceTestCase):
    ...
    def test_new_york_offsets_follow_daylight_saving(self):
        self.assertEqual(
            common.at_et(date(2026, 10, 30), time(12, 0)).isoformat(),
            "2026-10-30T12:00:00-04:00",
        )
        # DST ends 2026-11-01; the next Monday is EST.
        self.assertEqual(
            common.at_et(date(2026, 11, 2), time(12, 0)).isoformat(),
    ...
    def test_reset_windows_are_jan_1_and_jun_1(self):
        self.assertEqual(
            common.reset_window(date(2026, 5, 31)),
            (date(2026, 1, 1), date(2026, 5, 31)),
        )
        self.assertEqual(
            common.reset_window(date(2026, 6, 1)),
            (date(2026, 6, 1), date(2026, 12, 31)),
        )
        self.assertEqual(
            common.reset_window(date(2026, 12, 31)),
            (date(2026, 6, 1), date(2026, 12, 31)),
        )
    ...
    def test_same_label_gets_the_join_month_then_the_join_day(self):
        a = self.make_employee(
            "Leo", "Alpha", "leo.a@example.com", joined=date(2026, 9, 2)
        )
        b = self.make_employee(
            "Leo", "Adams", "leo.b@example.com", joined=date(2026, 10, 5)
        )
        c = self.make_employee(
            "Leo", "Avery", "leo.c@example.com", joined=date(2026, 10, 9)
        )
        d = self.make_employee(
            "Leo", "Arden", "leo.d@example.com", joined=date(2026, 10, 9)
        )
    ...
    def test_labels_are_unique_across_the_whole_floor_even_for_a_subset(self):
        a = self.make_employee(
            "Ana", "Bell", "ana.b@example.com", position=27, joined=date(2026, 7, 1)
        )
        self.make_employee(
            "Ana", "Brown", "ana.c@example.com", position=28, joined=date(2026, 8, 1)
        )
