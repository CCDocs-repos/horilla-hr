...
from horilla.ccdocs_attendance.tests.base import API_TOKEN, LINK_TOKEN, AttendanceTestCase
...
class ApiTestCase(AttendanceTestCase):
    ...
    def post_json(self, path, body, token=API_TOKEN):
        return self.client.post(
            API + path, json.dumps(body), content_type="application/json", **self.api_headers(token)
        )

    def notice(self, employee, first, last=None, kind="out", filed_at=None, status="requested"):
...
class AuthTests(ApiTestCase):
    ...
    def test_401_without_or_with_a_wrong_token(self):
        ...
        self.assertEqual(response.status_code, 401)
        response = self.post_json("deliveries/claim/", {"key": "k", "kind": "late_email"}, token="nope")
        self.assertEqual(response.status_code, 401)
    ...
    def test_wrong_method_and_bad_input_are_json_errors(self):
        ...
        response = self.client.post(
            API + "day-results/", "not json", content_type="application/json", **self.api_headers()
        )
        self.assertEqual(response.status_code, 400)
...
class DayReadTests(ApiTestCase):
    def setUp(self):
        ...
        self.agent = self.make_employee(
            "Quinn", "Del Rey", "quinn@example.com", position=27, shift=self.floor_shift,
            work_email="quinn.work@example.com",
        )
        self.lead = self.make_employee("Lee", "Lead", "lee@example.com", position=28)
        self.office = self.make_employee("Olga", "Office", "olga@example.com", position=90)
        self.linked_office = self.make_employee("Lin", "Linked", "lin@example.com", position=90)
        self.gone = self.make_employee("Gus", "Gone", "gus@example.com", position=27, active=False)
        AgentLink.objects.create(employee=self.agent, dialer_user="9001", valid_from=date(2026, 6, 2))
        AgentLink.objects.create(
            employee=self.agent, dialer_user="1111", valid_from=date(2026, 1, 1), valid_to=date(2026, 5, 31)
        )
        AgentLink.objects.create(employee=self.linked_office, dialer_user="2001", valid_from=date(2026, 9, 1))

    def test_roster_shift_links_and_urls(self):
        body = self.get("day/", date=FRIDAY.isoformat(), include=str(self.office.id)).json()
        self.assertEqual(body["form_url"], f"https://hr.example.test/attendance-notice/{LINK_TOKEN}/")
        self.assertEqual(body["manage_url"], "https://hr.example.test/ccdocs-attendance/manage/")
        by_id = {e["id"]: e for e in body["employees"]}
        self.assertEqual(
            set(by_id), {self.agent.id, self.lead.id, self.office.id, self.linked_office.id}
        )
        ...
        self.assertEqual(
            agent["shift"],
            {"id": self.floor_shift.id, "name": "Floor 12-8 ET", "start": "12:00", "end": "20:00"},
        )
    ...
    def test_unknown_include_is_404(self):
        self.assertEqual(self.get("day/", date="2026-09-25", include="999999").status_code, 404)
    ...
    def test_notices_results_history_and_points(self):
        covering = self.notice(self.agent, FRIDAY, FRIDAY + timedelta(days=3), kind="out")
        self.notice(self.agent, FRIDAY, status="void")
        self.notice(self.agent, FRIDAY + timedelta(days=5))
        DayResult.objects.create(
            employee=self.agent, day=FRIDAY, status="late", scheduled_start=time(12, 0),
            first_login_at=datetime(2026, 9, 25, 16, 7, 31, tzinfo=dt_timezone.utc), minutes_late=7,
        )
        thursday = FRIDAY - timedelta(days=1)
        early = common.at_et(thursday, time(10, 59, 59))
        self.notice(self.agent, thursday, filed_at=early)
        DayResult.objects.create(
            employee=self.agent, day=thursday, status="out", scheduled_start=time(12, 0), closed=True
        )
        wednesday = FRIDAY - timedelta(days=2)
        DayResult.objects.create(
            employee=self.agent, day=wednesday, status="out", scheduled_start=time(12, 0), closed=True
        )
        PointEntry.objects.create(
            employee=self.agent, day=wednesday, rule_key="out_no_notice", points=Decimal("2"),
            idem_key=f"{wednesday}:{self.agent.id}:out_no_notice",
        )
        PointEntry.objects.create(
            employee=self.agent, day=thursday, rule_key="out_notice", points=Decimal("1"),
            idem_key=f"{thursday}:{self.agent.id}:out_notice",
            voided_at=timezone.now(), voided_by="someone",
        )
        PointEntry.objects.create(
            employee=self.agent, day=date(2026, 5, 29), rule_key="late", points=Decimal("1"),
        ...
        self.assertEqual(
            ...
            [
                {
                    "employee_id": self.agent.id, "day": "2026-09-25", "status": "late",
                    "scheduled_start": "12:00", "first_login_at": "2026-09-25T12:07:31-04:00",
                    "minutes_late": 7, "notice_id": None, "closed": False,
                }
            ],
        )
        ...
        self.assertEqual((points["window_start"], points["window_end"]), ("2026-06-01", "2026-12-31"))
        self.assertEqual(points["total"], 2.0)
        self.assertEqual(len(points["entries"]), 2)
        self.assertEqual(
            {e["idem_key"]: e["voided"] for e in points["entries"]},
            {f"{wednesday}:{self.agent.id}:out_no_notice": False, f"{thursday}:{self.agent.id}:out_notice": True},
        )
    ...
    def test_called_in_uses_new_york_time_after_dst_ends(self):
        ...
        for employee, filed in (
            ...
            DayResult.objects.create(employee=employee, day=day, status="out", scheduled_start=time(12, 0))
...
class DayResultsWriteTests(ApiTestCase):
    ...
    def body(self, status="late", points=1.0, **extra):
        result = {
            "employee_id": self.agent.id, "status": status, "scheduled_start": "12:00",
            "first_login_at": "2026-09-25T12:07:31-04:00", "minutes_late": 7, "notice_id": None,
            "closed": False, "detail": {"source": "dialer", "login": "9001"},
            "points": [{"idem_key": self.key, "rule_key": "late", "points": points}] if points is not None else [],
        }
        result.update(extra)
        return {"date": FRIDAY.isoformat(), "run_id": "watch-2026-09-25T12:15", "results": [result]}

    def bad_rule_body(self):
        body = self.body()
        body["results"][0]["points"] = [
            {"idem_key": f"{FRIDAY}:{self.agent.id}:tardy", "rule_key": "tardy", "points": 1}
        ]
        return body
    ...
    def test_first_post_creates_and_the_repeat_is_a_no_op(self):
        ...
        self.assertEqual(
            first.json(),
            {"upserted": 1, "points_created": 1, "points_skipped_voided": 0, "skipped_excused": 0, "conflicts": []},
        )
        ...
        self.assertEqual(common.iso_et(result.first_login_at), "2026-09-25T12:07:31-04:00")
        entry = PointEntry.objects.get()
        self.assertEqual((entry.points, entry.source, entry.day_result_id), (Decimal("1.00"), "engine", result.id))

    def test_a_voided_point_is_never_revived(self):
        self.post_json("day-results/", self.body())
        PointEntry.objects.update(voided_at=timezone.now(), voided_by="sarthak@example.com", void_reason="wrong")
    ...
    def test_a_changed_value_is_a_conflict_and_nothing_changes(self):
        ...
        self.assertEqual(
            response.json()["conflicts"], [{"idem_key": self.key, "existing": 1.0, "incoming": 2.0}]
        )
        self.assertEqual(PointEntry.objects.get().points, Decimal("1.00"))

    def test_a_day_a_person_excused_is_never_overwritten(self):
        DayResult.objects.create(
            employee=self.agent, day=FRIDAY, status="excused", excused_by="fixer@example.com"
        )
        response = self.post_json("day-results/", self.body())
    ...
    def test_the_whole_post_is_one_transaction(self):
        ...
        bad = dict(good, employee_id=other.id, points=[{"idem_key": "wrong", "rule_key": "late", "points": 1}])
        response = self.post_json("day-results/", {"date": FRIDAY.isoformat(), "results": [good, bad]})
        self.assertEqual(response.status_code, 400)
    ...
    def test_input_is_checked(self):
        ...
        for body, status in cases:
            with self.subTest(body=body):
                self.assertEqual(self.post_json("day-results/", body).status_code, status)
        self.assertEqual(DayResult.objects.count(), 0)
    ...
    def test_first_login_keeps_the_est_offset_after_dst(self):
        ...
        body["results"][0]["points"] = [
            {"idem_key": f"2026-11-02:{self.agent.id}:late", "rule_key": "late", "points": 1}
        ]
        ...
        self.assertEqual(read["day_results"][0]["first_login_at"], "2026-11-02T12:07:31-05:00")

    def test_withdraw_voids_as_engine_and_lists_missing(self):
        self.post_json("day-results/", self.body())
        response = self.post_json(
            "points/withdraw/",
            {"idem_keys": [self.key, "2026-09-25:1:late"], "reason": "outage hold 2026-09-25", "run_id": "r1"},
        )
        self.assertEqual(response.json(), {"voided": 1, "missing": ["2026-09-25:1:late"]})
        entry = PointEntry.objects.get()
        self.assertEqual((entry.voided_by, entry.void_reason), ("engine", "outage hold 2026-09-25"))
        again = self.post_json("points/withdraw/", {"idem_keys": [self.key], "reason": "again"})
        self.assertEqual(again.json(), {"voided": 0, "missing": []})
        self.assertEqual(PointEntry.objects.get().void_reason, "outage hold 2026-09-25")
        self.assertEqual(self.post_json("points/withdraw/", {"idem_keys": [self.key]}).status_code, 400)
...
class LinksTests(ApiTestCase):
    ...
    def open(self, employee, login, valid_from="2026-09-01"):
        return {"employee_id": employee.id, "dialer_user": login, "webwork_email": None, "valid_from": valid_from}
    ...
    def test_one_open_holder_per_login(self):
        ...
        self.assertEqual(clash["conflicts"], [{"dialer_user": "9001", "held_by": self.ann.id}])
        self.assertEqual(AgentLink.objects.count(), 1)

    def test_close_then_reopen_for_someone_else_in_one_call(self):
        link_id = self.post_json("links/", {"open": [self.open(self.ann, "9001")]}).json()["opened"][0]
        response = self.post_json(
            "links/",
            {
                "close": [{"link_id": link_id, "valid_to": "2026-09-24", "reason": "dialer name changed"}],
        ...
        self.assertEqual((closed.valid_to, closed.close_reason), (date(2026, 9, 24), "dialer name changed"))

    def test_bad_links_are_refused(self):
        self.assertEqual(
            self.post_json("links/", {"open": [{"employee_id": self.ann.id, "valid_from": "2026-09-01"}]}).status_code,
            400,
        )
        self.assertEqual(
            self.post_json("links/", {"close": [{"link_id": 999999, "valid_to": "2026-09-24"}]}).status_code, 404
        )


class ShiftsDefaultTests(ApiTestCase):
    def test_only_floor_people_with_no_shift_get_the_default(self):
        self.assertEqual(self.post_json("shifts/default/", {"employee_ids": []}).status_code, 409)
        ...
        has_shift = self.make_employee("Hal", "Has", "hal@example.com", position=27, shift=other_shift)
        lead = self.make_employee("Lou", "Lead", "lou@example.com", position=28)
        gone = self.make_employee("Gil", "Gone", "gil@example.com", position=27, active=False)
        response = self.post_json(
            "shifts/default/", {"employee_ids": [no_shift.id, has_shift.id, lead.id, gone.id, 999999]}
        ).json()
        self.assertEqual(response["assigned"], [no_shift.id])
        self.assertEqual(
            {s["id"]: s["reason"] for s in response["skipped"]},
            {has_shift.id: "has_shift", lead.id: "not_floor_position", gone.id: "inactive", 999999: "not_found"},
        )
        shifts = dict(EmployeeWorkInformation.objects.entire().values_list("employee_id", "shift_id"))
...
class DeliveriesTests(ApiTestCase):
    def test_claim_once_then_409(self):
        claim = {"key": "late:2026-09-25:900", "kind": "late_email", "detail": {"to": "agent"}}
        first = self.post_json("deliveries/claim/", claim)
        self.assertEqual(first.status_code, 201)
        self.assertEqual(first.json(), {"key": "late:2026-09-25:900", "status": "attempting"})
    ...
    def test_done_and_stale_list(self):
        ...
        Delivery.objects.filter(key="a").update(updated_at=timezone.now() - timedelta(minutes=30))
        stale = self.get("deliveries/", status="attempting", older_than_minutes="15").json()
        self.assertEqual([d["key"] for d in stale["deliveries"]], ["a"])
        done = self.post_json("deliveries/done/", {"key": "a", "status": "sent", "detail": {"message_id": "m1"}})
        ...
        self.assertEqual(self.post_json("deliveries/done/", {"key": "a", "status": "sent"}).status_code, 200)
        self.assertEqual(self.post_json("deliveries/done/", {"key": "a", "status": "failed"}).status_code, 409)
        self.assertEqual(self.post_json("deliveries/done/", {"key": "zzz", "status": "sent"}).status_code, 404)
        self.assertEqual(self.post_json("deliveries/claim/", {"key": "c", "kind": "fax"}).status_code, 400)
