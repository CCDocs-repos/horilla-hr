...
class SetupCommandTests(AttendanceTestCase):
    def setUp(self):
        ...
        self.wrong_shift = self.make_shift(
            "Day Shift (9-6)", start=time(9), end=time(18)
        )
        ...
        self.viewer = self.make_employee(
            "Vic",
            "Viewer",
            "vic.personal@example.test",
            position=90,
            work_email="vic@example.test",
        )
    ...
    def state(self):
        return (
            ...
            sorted(
                Group.objects.filter(user__isnull=False).values_list("name", "user__id")
            ),
        )
    ...
    def test_dry_run_writes_nothing_and_says_what_it_would_do(self):
        ...
        self.assertIn(
            f"employee {self.wrong.id}: shift {self.wrong_shift.id} -> 'Floor 12-8 ET'",
            out,
        )
    ...
    def test_apply_sets_everything_up_and_a_second_apply_changes_nothing(self):
        ...
        self.assertTrue(
            Group.objects.get(name=common.FIXERS_GROUP)
            .user_set.filter(pk=self.fixer.employee_user_id.pk)
            .exists()
        )
        self.assertTrue(
            Group.objects.get(name=common.VIEWERS_GROUP)
            .user_set.filter(pk=self.viewer.employee_user_id.pk)
            .exists()
        )
        self.assertEqual(
            Contract.objects.entire().count(),
            contracts,
            "setup must not create payroll contracts",
        )
        ...
        change = next(
            c for c in data["work_info_changes"] if c["employee_id"] == self.wrong.id
        )
        self.assertEqual(change["previous_shift_id"], self.wrong_shift.id)

        before = self.state()
        out = self.run_setup(
            apply=True, snapshot=os.path.join(self.tmp, "setup-2.json")
        )
    ...
    def test_a_bad_id_or_unknown_email_stops_the_apply(self):
        ...
        with self.assertRaises(CommandError):
            self.run_setup(
                f"--late-shift-ids={self.lead.id}", apply=True, snapshot=snapshot
            )
        with self.assertRaises(CommandError):
            self.run_setup(
                "--fixers=nobody@example.test", apply=True, snapshot=snapshot
            )
    ...
    def test_revert_restores_and_removes_what_it_made(self):
        ...
        out = StringIO()
        call_command(
            "ccdocs_attendance_setup", f"--revert={snapshot}", "--apply", stdout=out
        )
        self.assertIn("Reverted: 4 shift(s) restored.", out.getvalue())
        self.assertEqual(self.shifts(), before_shifts)
        self.assertEqual(
            list(EmployeeShift.objects.values_list("id", flat=True)),
            [self.wrong_shift.id],
        )
        self.assertEqual(
            EmployeeShiftSchedule.objects.filter(shift_id=self.wrong_shift).count(), 5
        )
        self.assertEqual(EmployeeShiftSchedule.objects.count(), 5)
        self.assertFalse(
            Group.objects.filter(
                name__in=[common.FIXERS_GROUP, common.VIEWERS_GROUP]
            ).exists()
        )
        ...
        self.assertTrue(
            WorkRecords.objects.filter(pk=older.pk).exists(),
            "dated before the snapshot: kept",
        )
    ...
    def test_revert_leaves_a_later_human_change_alone(self):
        ...
        self.run_setup(apply=True, snapshot=snapshot)
        late = EmployeeShift.objects.get(employee_shift=common.LATE_SHIFT_NAME)
        EmployeeWorkInformation.objects.entire().filter(employee_id=self.a).update(
            shift_id=late
        )
        with self.assertRaises(CommandError):
